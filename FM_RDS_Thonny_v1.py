...
# Split branches
x_audio = np.diff(np.unwrap(np.angle(x)))  # Branch 1: Audio
x_rds = x.copy()  
...
x_audio *= 32767
x_audio = x_audio.astype(np.int16)
wavfile.write('fm.wav', int(sample_rate / 6), x_audio)
...
# Quadrature Demod
x = 0.5 * np.angle(x_rds[0:-1] * np.conj(x_rds[1:])) #Freq shift
N = len(x)
